...
router.get(
  ...
  passport.authenticate('jwt', { session: false }),
  (req, res) => {
    Contact.find({ ownerId: req.user._id }).then(contacts =>
      res.json(contacts)
    );
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    Contact.findOne({ email: req.body.email, ownerId: req.user._id }).then(
      contact => {
        if (contact)
          return res
            .status(400)
            .json({ email: 'A contact exists with this email' });
